...
import { saveCaseToDB } from '../lib/cases'
...
const NOMS = ['Dupont', 'Martin', 'Bernard', 'Dubois', 'Thomas', 'Robert', 'Leroy', 'Moreau', 'Simon', 'Laurent', 'Michel', 'Lefebvre', 'Lambert', 'Bonnet', 'François', 'Martinez', 'David', 'Bertrand', 'Roux', 'Vincent', 'Fournier', 'Morel', 'Girard', 'André', 'Lefèvre', 'Garnier', 'Chevalier', 'François', 'Legrand', 'Gauthier', 'Pascal', 'Morin', 'Mercier', 'Petit', 'Brun']
...
const PRENOMS_F = ['Marie', 'Françoise', 'Catherine', 'Sylvie', 'Nathalie', 'Monique', 'Isabelle', 'Martine', 'Josette', 'Nicole', 'Suzanne', 'Micheline', 'Claire', 'Hélène', 'Anne', 'Marguerite', 'Jacqueline', 'Colette', 'Denise', 'Yvette', 'Ginette', 'Liliane', 'Madeleine', 'Régine', 'Christiane', 'Léone', 'Simone', 'Lucienne', 'Paulette', 'Raymonde']
const ACTES = ['PAC (Pontage coronarien)', 'PAC x3', 'PAC x4', 'Remplacement valve mitrale', 'Réparation valve mitrale', 'Remplacement valve aortique', 'Réparation valve aortique', 'Remplacement valvulaire triple', 'CLOS (Closure of ASD/VSD)', 'Myxome atrial gauche', 'Transplantation cardiaque', 'Correction de Fallot', 'TAVI (Valve aortique transcatheter)', 'Switch artériel (Nikaidoh)', 'Coarctation aortique', 'Anévrisme aortique ascendant', 'Endocardite infectieuse', 'Cœur batriloculaire', 'Sténose mitrale', 'Insuffisance tricuspide']
...
const CATS_OUT = ['Diurèse', 'UF', 'Pertes sanguines', 'Sangشتنه']
...
function generateParamHistory(startTime: string, dureeCEC: number) {
  const entries = []
  const nbEntries = Math.min(dureeCEC, randInt(15, 60))
  for (let i = 0; i < nbEntries; i++) {
    ...
    entries.push({
      ...
      debit: rand(3.0, 6.0),
      pam: randInt(55, 90),
      temperature: rand(34.0, 37.5),
      hct: rand(22, 38),
      pao2: rand(150, 350),
      hb: rand(7, 14),
      lactates: rand(0.5, 4.0),
      k: rand(3.5, 5.5),
    })
  }
  return entries
...
function generateOneCase(_index: number): CaseData {
  const id = uuidv4()
  const patientId = uuidv4()
  const sexe = pick(['M', 'F']) as 'M' | 'F'
  ...
  const nbEntrees = randInt(4, 8)
  const nbSorties = randInt(3, 6)
  const bilan = []
  for (let i = 0; i < nbEntrees; i++) {
    bilan.push({ id: uuidv4(), heure: `${String(randInt(8, 18)).padStart(2, '0')}:${String(randInt(0, 59)).padStart(2, '0')}`, type: 'entree' as const, categorie: pick(CATS_IN), volume: randInt(100, 1500) })
  }
  for (let i = 0; i < nbSorties; i++) {
    bilan.push({ id: uuidv4(), heure: `${String(randInt(8, 18)).padStart(2, '0')}:${String(randInt(0, 59)).padStart(2, '0')}`, type: 'sortie' as const, categorie: pick(CATS_OUT), volume: randInt(50, 800) })
  }

  const nbPrime = randInt(2, 5)
  const primeComp = []
  const usedSol = new Set<string>()
  for (let i = 0; i < nbPrime; i++) {
    let s: string
  ...
  const nbAdminCardio = randInt(1, 5)
  const administrations = []
  for (let i = 0; i < nbAdminCardio; i++) {
  ...
  return {
    ...
    patient: {
      id: patientId,
      nom: pick(NOMS),
      prenom: sexe === 'M' ? pick(PRENOMS_M) : pick(PRENOMS_F),
      dateNaissance: randomDate(1940, 2010),
      poids,
      taille,
      sexe,
      groupeSanguin: pick(['A+', 'A-', 'B+', 'B-', 'AB+', 'O+', 'O-']),
      numDossier: String(randInt(100000, 999999)),
      asa: pick(['I', 'II', 'III', 'IV', 'V']),
    },
    intervention: {
      date: dateIntervention,
      type: pick(ACTES),
      chirurgien: pick(CHIRURGIENS),
      anesthesiste: pick(ANESTHESISTES),
      perfusionniste: pick(PERFUSIONNISTES),
      assistant: pick(CHIRURGIENS),
    },
    materiel: {
      oxygateur: pick(OXYGATEURS),
      circuit: pick(['套装 Adulte', '套装 Pediatric', '套装 Neonatal']),
      canuleArterielle: pick(CANULES_ART),
      canuleVeineuse: pick(CANULES_VEIN),
      volumePrime: randInt(1000, 2500),
      primeComposition: primeComp,
    },
    parametres: {
      debit: rand(3.0, 6.5),
      pam: randInt(50, 95),
      temperature: rand(33.0, 37.5),
      hct: rand(20, 40),
      sao2: rand(95, 100),
      svo2: rand(55, 85),
      pao2: rand(120, 400),
      ...
      hb: rand(6, 15),
      k: rand(3.0, 6.0),
      lactates: rand(0.3, 5.0),
      glycemie: rand(3.5, 12),
    },
    cardioplegie: {
      type: pick(TYPES_CARDIO) as CaseData['cardioplegie']['type'],
      voie: pick(VOIES_CARDIO) as CaseData['cardioplegie']['voie'],
      volume: randInt(200, 1500),
      concentration: pick(['KCl 20 mEq/L', 'KCl 30 mEq/L', 'MgSO4 1g/L', '']),
      temperature: rand(4, 20),
      arretAortique: Math.random() > 0.3,
      administrations,
    },
    checklistPre: [
...
export async function generateTestCases(count: number = 200, onProgress?: (current: number, total: number) => void): Promise<CaseData[]> {
  const cases: CaseData[] = []
  for (let i = 0; i < count; i++) {
    cases.push(generateOneCase(i))
    onProgress?.(i + 1, count)
  }
  return cases
}

export async function injectTestCases(cases: CaseData[], onProgress?: (current: number, total: number) => void): Promise<void> {
  for (let i = 0; i < cases.length; i++) {
    await saveCaseToDB(cases[i])
